...
def _bootstrap() -> None:
    """Make the app robust when run from a source checkout (no pip install)."""

    # Use a dedicated Numba cache dir so cached kernels from other installs/paths
    # don't break unpickling (this is a common source of ModuleNotFoundError).
    try:
        cache_dir = Path.home() / ".labelis_render_only" / "numba_cache"
        cache_dir.mkdir(parents=True, exist_ok=True)
        os.environ.setdefault("NUMBA_CACHE_DIR", str(cache_dir))
    except Exception:
        # If anything goes wrong, we just don't force the cache dir.
        pass

    # Ensure `labelis` is importable if the user extracted this folder next to `labelis/`.
    try:
        import labelis  # noqa: F401
        return
    except Exception:
        pass
    ...
    for p in [here.parent] + list(here.parents):
        if (p / "labelis" / "__init__.py").exists():
...
from labelis.pipeline.io import load_localizations  # noqa: E402
from labelis.pipeline.render import render_dispatch  # noqa: E402
...
class RenderOnlyDockWidget(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.chk_sigma_is_variance = QtWidgets.QCheckBox(
            "Sigma column is variance (Labelis/MATLAB-compatible)"
        )
...
class RenderOnlyController(QtCore.QObject):
    ...
    def _render_error(self, tb: str) -> None:
        ...
        # Friendly hint for the specific failure you reported
        if "numba" in tb.lower() and "ModuleNotFoundError" in tb and "labelis" in tb:
            hint = (
                "Numba cache load failed because the cached kernel was compiled under a different import path.\n\n"
                "Fix options:\n"
                "1) Re-run from the folder that contains the `labelis/` package (or install labelis).\n"
                "2) Delete your Numba cache (or let this app use its own cache dir via NUMBA_CACHE_DIR).\n"
                "3) Switch engine to `turbo_bin_blur` (fast preview) if you just need a quick render."
            )
            tb = hint + "\n\n---\n\n" + tb
