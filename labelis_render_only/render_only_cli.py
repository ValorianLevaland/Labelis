...
def _bootstrap() -> None:
    # Dedicated numba cache dir (avoids cache collisions between different Labelis paths)
    try:
    ...
    # Allow running next to a source checkout without `pip install -e .`
    try:
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Render a Labelis-style reconstruction from a localization CSV/TSV and save as TIFF."
    )
